 ...
 * @author DAT
 */
public class LoadOnsaleLists extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet LoadOnsaleLists</title>");  
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet LoadOnsaleLists at " + request.getContextPath () + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    } 
    ...
    try {
        ...
        int totalProducts = productDAO.getTotalFilteredSaleProducts(category, brand, lowPrice, highPrice, query);

        // Calculate total pages based on the number of products
        int totalPages = (int) Math.ceil((double) totalProducts / pageSize);
        List<Products> productList = productDAO.getAllSaleProduct(category, brand, lowPrice, highPrice, query, pageSize, offset);
