 ...
 * @author DAT
 */
public class LoadProductListStaff extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet LoadProductListStaff</title>");  
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet LoadProductListStaff at " + request.getContextPath () + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    } 
  ...
  protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    ...
    int totalProducts = productDAO.getTotalFilterProductsStaff(category, status, query);
    int totalPages = (int) Math.ceil((double) totalProducts / productsPerPage);

    // Fetch the products for the current page
    List<Products> products = productDAO.getAllFilterProductsStaff(category, status, query, productsPerPage, (page-1)*productsPerPage);
    ...
    for (Products product : products) {
        ...
        out.println("<td style='color:" + (product.getStatus().equals("inactive") ? "red" : "green") + "'>" 
                    + (product.getStatus().equals("active") ? "Active" : "Inactive") + "</td>");
        out.println("<td><button class='details-btn' onclick='showProductDetails(\"" + product.getId() + "\", \"" 
            + product.getLink_picture() + "\", \"" + product.getCategory() + "\", \"" + product.getBrand() 
            + "\", \"" + product.getName() + "\", \"" + product.getDetails() + "\", \"" + product.getQuantity() 
            + "\", \"" + product.getPrice() + "\", \"" + product.getSalePercent() + "\", \"" + product.getStatus() 
            + "\", \"" + product.getColor() + "\", \"" + product.getSize() + "\")'>Details</button></td>");
        out.println("</tr>");
    }
